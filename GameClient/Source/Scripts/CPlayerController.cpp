...
CPlayerController::CPlayerController()
	: Super(SCRIPT_TYPE::PLAYERCONTROLLER), m_animOffset(UINT(ARROW_DIR::END))
{
...
{
	// Editor Play 시 m_anim은 복사 생성된 AtlasAnimator로 변해야 한다.	
	m_anim = dynamic_cast<CAtlasAnimator*>(GetOwner()->GetComponent(COMPONENT_TYPE::ATLASANIMATOR).Get());
	m_anim->Play(m_idleAnimKey, true);
}

void CPlayerController::PlayAnimation(ARROW_DIR dir, bool isFailed, bool autoIdle)
{
	int idx = 0;
	if (isFailed)
	{
...
void CPlayerController::SaveToSceneFile(FILE* _File)
{
}

void CPlayerController::LoadFromSceneFile(FILE* _File)
{
}
