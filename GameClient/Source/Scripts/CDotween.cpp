...
CDotween::CDotween()
	: CScript(SCRIPT_TYPE::DOTWEEN)
{
	for (int i = 0; i < UINT(UINT(TimerIndex::End)); i++)
	{
		dotweenTimerVector.push_back(new CDotweenTimer);
	}
}
...
void CDotween::StopAllDotweenFunction()
{
...
Ptr<CDotween> CDotween::DOMove(Vec3 endPosition, double p_duration)
{
	// 	GetOwner()->GetTransform()는 Component를 갖게 될 GameObject를 리턴한다.
	CDotweenTimer* m_doMovetweenTimer = dotweenTimerVector[UINT(TimerIndex::MoveTimer)].Get();
...
Ptr<CDotween> CDotween::DOScale(Vec3 endScale, double p_duration)
{
	CDotweenTimer* m_doScaletweenTimer = dotweenTimerVector[UINT(TimerIndex::ScaleTimer)].Get();
...
Ptr<CDotween> CDotween::DORotate(Vec3 endRotation, double p_duration)
{
	/// DORotate 만들 때 주의할 점.
	/// 1. 각이 360을 넘어가면 0으로 초기화 해준다. 
	/// 일단 여기까지?
	CDotweenTimer* m_doRotatetweenTimer = dotweenTimerVector[UINT(TimerIndex::RotateTimer)].Get();
...
Ptr<CDotween> CDotween::DOQRotate(Vec3 axis, double angle, double p_duration)
{
	CDotweenTimer* m_doQrotatetweenTimer = dotweenTimerVector[UINT(TimerIndex::RotateTimer)].Get();
...
/// <returns></returns>
Ptr<CDotween> CDotween::DOLookAt(Vec3 lookTransform, double p_duration, bool isYaxisInclude)
{
	//StopAllDotweenFunction();
	//CDotweenTimer* m_doLookTweenTimer = dotweenTimerVector[TimerIndex::RotateTimer];
...
/// <returns></returns>
Ptr<CDotween> CDotween::DOCustom(double p_duration)
{
	CDotweenTimer* m_doCustomTweenTimer = dotweenTimerVector[UINT(TimerIndex::CustomTimer)].Get();
...
Ptr<CDotween> CDotween::DONothing(double p_duration)
{
...
Ptr<CDotween> CDotween::DOShakePosition(double p_duration, double strength /*= 1.5f*/, int vibrato /*= 1.5*/)
{
	CDotweenTimer* m_doShaketweenTimer = dotweenTimerVector[UINT(TimerIndex::ShakeTimer)].Get();
...
Ptr<CDotween> CDotween::SetDelay(double p_delay)
{
...
Ptr<CDotween> CDotween::IsRepeat(bool p_repeat)
{
	dotweenTimerVector[UINT(currentTimerIndex)]->isRepeat = p_repeat;
	return this;
}

Ptr<CDotween> CDotween::SetEase(easing_functions p_ease)
{
...
Ptr<CDotween> CDotween::OnComplete(std::function<void()> expirationFunc)
{
	dotweenTimerVector[UINT(currentTimerIndex)]->onCompleteFunction = expirationFunc;
	return this;
}

Ptr<CDotween> CDotween::OnUpdate(std::function<void()> updateFunc)
{
	dotweenTimerVector[UINT(currentTimerIndex)]->onUpdate = updateFunc;
	return this;
}

double CDotween::AdjustRotation(float& rot)
{
	while (rot > 360 || rot < 0)
	{
		if (rot < 0)
...
void CDotween::SaveToSceneFile(FILE* _File)
{
}

void CDotween::LoadFromSceneFile(FILE* _File)
{
}

//void CDotween::clearDotweenTimerMap()
//{
