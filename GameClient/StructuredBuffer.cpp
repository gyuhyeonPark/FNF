	...
	, m_SysMemMove(_Origin.m_SysMemMove)
{
	...
	if (nullptr != _Origin.m_SB)
		DEVICE_CONTEXT->CopyResource(m_SB.Get(), _Origin.m_SB.Get());
}
...
void StructuredBuffer::SetData(void* _SysMem, UINT _DataSize)
{
	...
	DEVICE_CONTEXT->Map(m_SB_Write.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapSub);
	memcpy(mapSub.pData, _SysMem, _DataSize);
	DEVICE_CONTEXT->Unmap(m_SB_Write.Get(), 0);

	// WirteBuffer -> MainBuffer
	DEVICE_CONTEXT->CopyResource(m_SB.Get(), m_SB_Write.Get());
}
...
void StructuredBuffer::GetData(void* _DstSysMem, UINT _DataSize)
{
	...
	// Main -> Read
	DEVICE_CONTEXT->CopyResource(m_SB_Read.Get(), m_SB.Get());

	// Read -> SysMem
	D3D11_MAPPED_SUBRESOURCE mapSub = {};
	DEVICE_CONTEXT->Map(m_SB_Read.Get(), 0, D3D11_MAP_READ, 0, &mapSub);
	memcpy(_DstSysMem, mapSub.pData, _DataSize);
	DEVICE_CONTEXT->Unmap(m_SB_Read.Get(), 0);
}
...
void StructuredBuffer::Binding(UINT _RegisterNum)
{
	...
	DEVICE_CONTEXT->VSSetShaderResources(_RegisterNum, 1, m_SRV.GetAddressOf());
	DEVICE_CONTEXT->HSSetShaderResources(_RegisterNum, 1, m_SRV.GetAddressOf());
	DEVICE_CONTEXT->DSSetShaderResources(_RegisterNum, 1, m_SRV.GetAddressOf());
	DEVICE_CONTEXT->GSSetShaderResources(_RegisterNum, 1, m_SRV.GetAddressOf());
	DEVICE_CONTEXT->PSSetShaderResources(_RegisterNum, 1, m_SRV.GetAddressOf());
}
...
void StructuredBuffer::Clear()
{
	...
	DEVICE_CONTEXT->VSSetShaderResources(m_RecentRegisterNum, 1, &pSRV);
	DEVICE_CONTEXT->HSSetShaderResources(m_RecentRegisterNum, 1, &pSRV);
	DEVICE_CONTEXT->DSSetShaderResources(m_RecentRegisterNum, 1, &pSRV);
	DEVICE_CONTEXT->GSSetShaderResources(m_RecentRegisterNum, 1, &pSRV);
	DEVICE_CONTEXT->PSSetShaderResources(m_RecentRegisterNum, 1, &pSRV);
	m_RecentRegisterNum = -1;
}
...
void StructuredBuffer::Binding_CS_SRV(UINT _RegisterNum)
{
	...
	DEVICE_CONTEXT->CSSetShaderResources(_RegisterNum, 1, m_SRV.GetAddressOf());
}

void StructuredBuffer::Clear_CS_SRV()
{
	ID3D11ShaderResourceView* pSRV = nullptr;
	DEVICE_CONTEXT->CSSetShaderResources(m_RecentSRVNum, 1, &pSRV);
	m_RecentSRVNum = -1;
}
...
void StructuredBuffer::Binding_CS_UAV(UINT _RegisterNum)
{
	...
	UINT i = -1;
	DEVICE_CONTEXT->CSSetUnorderedAccessViews(_RegisterNum, 1, m_UAV.GetAddressOf(), &i);
}
...
void StructuredBuffer::Clear_CS_UAV()
{
	...
	UINT i = -1;
	DEVICE_CONTEXT->CSSetUnorderedAccessViews(m_RecentUAVNum, 1, &pUAV, &i);
	m_RecentUAVNum = -1;
}
