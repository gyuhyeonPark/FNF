...
{
	for (const auto& Object : _Origin.m_parentVec)
	{
...
void Layer::DeregisterAsParent(Ptr<GameObject> obj)
{
	vector<Ptr<GameObject>>::iterator iter = m_parentVec.begin();
	for (; iter != m_parentVec.end(); ++iter)
	{
		if (*iter == obj)
		{
			m_parentVec.erase(iter);
			return;
		}
	}
