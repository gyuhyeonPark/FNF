...
RenderManager::RenderManager()
	: m_debugOption(false), m_mainCamera{nullptr}
{
...
void RenderManager::Init()
{
	...
	m_dbgObj->GetMeshRenderer()->SetMaterial(FIND(AMaterial, L"DbgMtrl"));
...
void RenderManager::Progress()
{
	...
	{
		// 카메라를 이용해서 씬 내부 물체들을 렌더링
		for (UINT i = 0; i < (UINT)CamIdx::END; ++i)
		{
			if (m_mainCamera[i] == nullptr)
				continue;
			m_mainCamera[i]->SortObject();
			m_mainCamera[i]->Render();
		}
	}
	else
	{
...
void RenderManager::RegisterCamera(Ptr<CCamera> cam, CamIdx _idx)
{
	m_mainCamera[(UINT)_idx] = cam;
}
...
void RenderManager::Render_Debug()
{
	...
	for (; iter != m_dbgInfoList.end();)
	{
		...
		// Material 설정
		m_dbgObj->MeshRenderer()->GetMaterial()->SetScalar(VEC4_0, (*iter).color);

		if ((*iter).depthTest)
			m_dbgObj->MeshRenderer()->GetMaterial()->GetShader()->SetDSType(DS_TYPE::LESS);
		else
			m_dbgObj->MeshRenderer()->GetMaterial()->GetShader()->SetDSType(DS_TYPE::NO_TEST_NO_WRITE);
