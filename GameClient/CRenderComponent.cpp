...
{
	// 원본 렌더러 컴포넌트가 공유재질(에셋 매니저로 관리되는)을 사용하고 있다면
	if (_Origin.m_Mtrl == _Origin.m_SharedMtrl)
	{
		// 나도 공유재질 사용
		m_Mtrl = m_SharedMtrl;
	}
	// 원본 렌더러 컴포넌트의 동적재질이 존재하고, 현재 사용중인 재질이 동적 재질인 경우
...
void CRenderComponent::SaveToSceneFile(FILE* _File)
{
...
void CRenderComponent::LoadFromSceneFile(FILE* _File)
{
...
{
	// 동적 재질 생성은 반드시 씬이 Play 모드일 경우에만 사용 가능한 기능.
	assert(SceneManager::GetInstance()->GetSceneState() == SCENE_STATE::PLAY);
